class Heap:
    ...
    def extract_max(self):
        ...
        if self.size == 1:
            ...
            self.heap.pop()
            return max
        if self.has_right_child(-2):
            if self.heap[-1][0] > self.heap[-2][0]:
                self.heap[-1] = self.heap[self.size]
                self.heap.pop()
                self.size -= 1
                return self.heap[-1][1]
            else:
                self.heap[-2] = self.heap[self.size]
                self.heap.pop()
                self.size -= 1
                return self.heap[-2][1]
...
if __name__ == '__main__':
    ...
    # h.insert((1, "d"))
    # h.insert((4, "e"))
    print(h.extract_max()) # "b"
    print(h.extract_max()) # "c"
    print(h.extract_max()) # "a"
